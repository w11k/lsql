...
public class TableTest extends AbstractLSqlTest {
    ...
    public void failOnWrongTableName() {
        createTable("CREATE TABLE namenamenamenamename (id_pk INTEGER PRIMARY KEY, age INT)");
        lSql.table("wrongwrongwrongwrongwrong");
    }

    @Test
    public void getById() {
        createTable("CREATE TABLE table1 (id_pk INTEGER PRIMARY KEY, age INT)");
        Table table1 = lSql.table("table1");

        table1.insert(Row.fromKeyVals("idPk", 1, "age", 1));
        table1.insert(Row.fromKeyVals("idPk", 2, "age", 2));
        table1.insert(Row.fromKeyVals("idPk", 3, "age", 3));
    ...
    public void insertRow() throws SQLException {
        createTable("CREATE TABLE table1 (id_pk INTEGER PRIMARY KEY, name TEXT)");
        Table table1 = lSql.table("table1");

        Row row = new Row().addKeyVals("idPk", 1, "name", "cus1");
        table1.insert(row);
    ...
    public void insertFailsOnWrongColumnName() throws SQLException {
        ...
        Row row = new Row().addKeyVals("idPk", 1, "nameTYPO", "cus1");
        table1.insert(row);
    }
    ...
    public void insertShouldPutIdIntoRowObject() {
        createTable("CREATE TABLE table1 (id_pk SERIAL PRIMARY KEY, age INT)");
        Table table1 = lSql.table("table1");
        Row row = new Row().addKeyVals("age", 1);
        Optional<Object> optional = table1.insert(row);
        assertTrue(optional.isPresent());
        assertEquals(optional.get(), row.get("idPk"));
    }
    ...
    public void updateShouldFailOnWrongKeys() throws SQLException {
        createTable("CREATE TABLE table1 (id_pk INTEGER PRIMARY KEY, name TEXT)");
        Table table1 = lSql.table("table1");
        Row row = new Row().addKeyVals("idPk", 1, "name", "Max");
        table1.insert(row);
    ...
    public void saveWithoutAutoIncrement() throws SQLException {
        ...
        // Insert
        Row row = Row.fromKeyVals("idPk", 1, "name", "Max");
        Object id = table1.save(row).get();
        assertEquals(id, row.get("idPk"));
    ...
    public void fetchColumns() throws SQLException {
        createTable("CREATE TABLE table1 (id_pk SERIAL PRIMARY KEY, name TEXT, age INT)");
        Table table1 = lSql.table("table1");
        assertEquals(table1.getColumns().size(), 3);
        assertTrue(table1.getColumns().containsKey("id_pk"));
